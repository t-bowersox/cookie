...
describe("Cookie", () => {
  ...
  test("it outputs a cookie string excluding Expires, Max-Age, Domain, & Path", () => {
    const expectedCookie = `id=testing123; SameSite=Lax; HttpOnly; Secure`;
    expect(cookie.toString()).toMatch(expectedCookie);
  });
  ...
  test("it parses a string with only a name & value to a cookie", () => {
    const cookieStr = `id=testing123`;
    const expectedCookie = new Cookie("id", "testing123")
      .setHttpOnly(false)
      .setSecure(false);
